...
### `ParallelComm`: `portUrb/model/core/ParallelComm.h`

This class manages MPI operations for you, allowing you to encapsulate (and create) an MPI sub-communicator, gather information on the encapsulated communicator, and perform operations on it as well such as halo exchanges and reductions of data over the involved MPI tasks. This covers the main functionality, but for more details, please see the source file where more detailed documentation is available.
* Constructors
   * There is an empty constructor and a constructor that takes in an `MPI_Comm` object to wrap. You can also use an empty constructor to create the ParallelComm object and then call the `void ParallelComm(MPI_Comm comm);` function to wrap a communicator.
* If you want to create a new MPI sub-communicator from a parent communicator, you can call `ParallelComm ParallelComm::create( bool b , MPI_Comm parent_comm = MPI_COMM_WORLD );` where each MPI task in the parent communicator calls this and passes a boolean, and the tasks that are `true` form the new communicator (and the `false` task communicator from the MPI_Comm_Split call is deleted). The boolean could be, for instance, whethe a certain region is part of the current MPI task's domain so that only MPI tasks within that region will have a valid `ParallelComm` object passed back. Any task with a `false` boolean will have `return_value.valid() == false`, and any task with a `true` boolean will encapsulate a new sub-communicator `ParallelComm` object that has `return_value.valid() == true`.
   * This is used to create ensembles in the `Ensembler` class to ensure that a given ensemble has a `ParallelComm` object with only the MPI tasks involved in that particular ensemble.
   * This is also used in the wind turbine LES parameterizations to ensure that for a given turbine, only the MPI tasks that are potentially involved with a given turbine's domain are included in the turbine's `ParallelComm` object to reduce the number of MPI tasks involved in reductions. 
* You can get the underlying communicator with `MPI_Comm ParallelComm::get_mpi_comm()`. You can get the number of MPI tasks involved in the communicator with `int ParallelComm::get_size()` or simple `size()`. You can get your process's rank ID within the given `ParallelComm` object's sub-communicator with `int ParallelComm::get_rank_id()`, and you can get the associated `MPI_Group` object for the `ParallelComm` object with `MPI_Group ParallelComm::get_group()`. 
* You can halt progress until all MPI tasks involved in the sub-communicator have completed their work with `void ParallelComm::barrier()`.
* There are also many different MPI operation wrappers such as send_recv, reduce, all_reduce, and broadcast implemented, and for examples of those, please see the `model/modules` source files.
* GPU-aware concerns are abstracted away from the user so that the user can focus on the operations themselves. 
