  ...
  struct WindmillActuators {
    ...
    struct TurbineGroup {
      ...
      void add_turbine( core::Coupler       & coupler     ,
        ...
        if (active) {
          ...
          bool owner = base_loc_x >= xdom_beg+i_beg*dx && base_loc_x < xdom_beg+(i_beg+nx)*dx &&
                       base_loc_y >= ydom_beg+j_beg*dy && base_loc_y < ydom_beg+(j_beg+ny)*dy;
            ...
            for (int kk=0; kk < N; kk++) {
              for (int jj=0; jj < N; jj++) {
                for (int ii=0; ii < N; ii++) {
                  real x = xdom_beg + (i_beg+i)*dx + ii*dx/(N-1);
                  real y = ydom_beg + (j_beg+j)*dy + jj*dy/(N-1);
                  real z = zmid(k) + kk*dz(k)/(N-1);
                  auto bx  = base_loc_x;
                  auto by  = base_loc_y;
    ...
    void init( core::Coupler &coupler ) {
      ...
      if (coupler.option_exists("turbine_x_locs") && coupler.option_exists("turbine_y_locs")) {
        ...
        for (real y = ydom_beg+yinc/2; y < ydom_beg+ylen; y += yinc) {
          for (real x = xdom_beg+xinc/2; x < xdom_beg+xlen; x += xinc) {
      ...
      for (int iturb = 0; iturb < turbine_group.turbines.size(); iturb++) {
        ...
        if (turbine.active) {
          ...
          if (turbine.apply_thrust && coupler.get_option<bool>("turbine_immerse_material",false)) {
            ...
            yakl::parallel_for( YAKL_AUTO_LABEL() , SimpleBounds<3>(nz,ny,nx) , KOKKOS_LAMBDA (int k, int j, int i) {
              ...
              for (int kk=0; kk < N; kk++) {
                for (int jj=0; jj < N; jj++) {
                  for (int ii=0; ii < N; ii++) {
                    F x = xdom_beg + (i_beg+i)*dx + ii*dx/(N-1);
                    F y = ydom_beg + (j_beg+j)*dy + jj*dy/(N-1);
          ...
          yakl::parallel_for( YAKL_AUTO_LABEL() , SimpleBounds<3>(nz,ny,nx) , KOKKOS_LAMBDA (int k, int j, int i) {
            ...
            F x = xdom_beg + (i_beg+i+0.5f)*dx;
            F y = ydom_beg + (j_beg+j+0.5f)*dy;
            F z = zmid(k);
