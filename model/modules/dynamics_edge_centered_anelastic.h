...
namespace modules {
  ...
  struct Dynamics_Euler_Stratified {
    ...
    void compute_tendencies( core::Coupler       & coupler      ,
      ...
      bool const cg_check = coupler.get_option<bool>("dycore_anelastic_check_cg_compatibility",true);
      bool use_cg = coupler.get_option<bool>("dycore_anelastic_use_cg",cg_check);
      if constexpr (yakl::kokkos_debug) {
      ...
      bool const linearity_check = coupler.get_option<bool>("dycore_anelastic_check_linearity",false);
      if constexpr (yakl::kokkos_debug) {
        if (linearity_check) {
      ...
      yakl::parallel_for(YAKL_AUTO_LABEL(), SimpleBounds<3>(nz,ny,nx), KOKKOS_LAMBDA (int k, int j, int i) {
        ...
        state_tend(idU,k,j,i) = (momentum_work(idRU,k,j,i)-state(idU,k,j,i))/dt;
        state_tend(idV,k,j,i) = (momentum_work(idRV,k,j,i)-state(idV,k,j,i))/dt;
        state_tend(idW,k,j,i) = (momentum_work(idRW,k,j,i)-state(idW,k,j,i))/dt;
