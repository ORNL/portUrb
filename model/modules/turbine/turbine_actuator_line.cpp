...
Node convert<std::tuple<real,real,real,std::string>>::encode(
    const std::tuple<real,real,real,std::string>& rhs) {
      Node node;
...
bool convert<std::tuple<real,real,real,std::string>>::decode(
    const Node& node, std::tuple<real,real,real,std::string>& rhs) {
...
void TurbineActuatorLine::RefTurbine::init( core::Coupler const & coupler ) {
...
void TurbineActuatorLine::Traces::register_entry( std::string name , bool dims_pnts ) {
        entries.push_back( {name,dims_pnts,std::vector<real>(),std::vector<realHost2d>()} );
      }

std::vector<real> & TurbineActuatorLine::Traces::get_1( std::string name ) {
...
std::vector<realHost2d> & TurbineActuatorLine::Traces::get_pnts( std::string name ) {
        for (auto & entry : entries) {
          if (name==entry.name) {
            if (entry.dims_pnts) {
...
void TurbineActuatorLine::Traces::clear_all() {
        for (auto & entry : entries) {
          entry.vals_1   .clear();
...
void TurbineActuatorLine::TurbineGroup::add_turbine( core::Coupler       & coupler     ,
                        real                  base_loc_x  ,
                        real                  base_loc_y  ,
...
void TurbineActuatorLine::init( core::Coupler &coupler ) {
...
void TurbineActuatorLine::apply( core::Coupler & coupler , real dt ) {
      using yakl::SimpleBounds;
      auto nx                = coupler.get_nx   ();  // Get the local number of x-direction cells
      auto ny                = coupler.get_ny   ();  // Get the local number of y-direction cells
      auto nz                = coupler.get_nz   ();  // Get the number of z-direction cells
