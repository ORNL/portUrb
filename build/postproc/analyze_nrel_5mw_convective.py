...
fig.tight_layout()
plt.tight_layout()
plt.savefig("turbine_contour_xz.png")
plt.show()
plt.close()
...
plt.tight_layout()
plt.grid()
plt.savefig("turbine_wake_y.png")
plt.show()
plt.close()
...
plt.tight_layout()
plt.grid()
plt.savefig("turbine_wake_z.png")
plt.show()
plt.close()

timevec = np.array([i/(len(pwr)-1)*len(times) for i in range(len(pwr))])
