...
int main(int argc, char** argv) {
  ...
  yakl::init();
  {
    ...
    turbines     .init        ( coupler );
    dycore       .init        ( coupler ); // Important that dycore inits after turbines for base immersed boundaries
    ...
    while (etime < sim_time) {
      ...
      // Run modules
      {
        ...
        coupler.run_module( [&] (Coupler &c) { turbines.apply          (c,dt); } , "turbines"      );
